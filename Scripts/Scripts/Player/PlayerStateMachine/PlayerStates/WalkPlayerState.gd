...
func handle_input(_event: InputEvent) -> void:
	if Input.is_action_pressed("input_wallrun"):
		if player.has_wall_on_front():
			...
			SideWallrunPlayerState.instance.set_right_wallrun_side()
			emit_signal("state_changing", SideWallrunPlayerState.instance)
		elif player.has_wall_on_left_side():
			SideWallrunPlayerState.instance.set_left_wallrun_side()
			emit_signal("state_changing", SideWallrunPlayerState.instance)
